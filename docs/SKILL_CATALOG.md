...
> 版本: v0.6 | 日期: 2026-06-01 | 状态: 数据模块重写（文件导入+真实统计）

本文档是 Artic 工作台所有内置技能的完整目录。按 9 大模块组织，每个条目标注所属分类、触发场景、以及与该技能相关的北极星指标指南。

## Artic 工作台 v0.5 概况

首页已升级为 SaaS 产品落地页，展示平台核心能力、功能面板速览、技能统计。进入 `/dashboard` 后使用 8 大功能面板进行运营管理。

| 导航菜单 | 功能定位 | 数据来源 |
|---|---|---|
| **工作台首页** | 关键指标仪表盘、待办聚合、最近动态、快速操作 | localStorage 各模块聚合 |
| **内容日历** | 月历排期规划、平台/状态管理（不承担发布） | 手动录入 |
| **活动管理** | 营销活动 CRUD、交付物追踪、进度条、关联素材库 | 手动录入 |
| **竞品追踪** | 结构化竞品画像、分类观察记录、并排对比表 | 手动录入 |
| **素材库** | 文案模板/品牌素材/SOP/截图/收藏、搜索、关联活动 | 手动录入 |
| **数据** | 文件导入（CSV/XLSX/DOCX）+ 数据对比 + AB 统计检验（z-test/chi-square）+ 归因看板 | 真实文件导入 + 真实统计 |
| **实验库** | 历史 AB 测试结果表、置信度、复用到新活动 | 手动录入 |
| **版本管理** | 版本时间线 + 环境快照预览滑块 | 手动录入 |

> **重要说明**：Artic 工作台定位为运营人员的数据驱动管理工具。竞品数据、内容排期、素材需手动录入，但数据模块支持真实文件导入（CSV/XLSX/DOCX）和统计检验（z-test/chi-square），可在本地完成数据分析和对比。不提供任何平台的自动抓取或发布功能——这些能力因平台反爬、API 权限等限制在本地工作台层面无法实现。
...
| 2026-06-01 | v0.4 务实重建：删除发布管道/内容工坊/链接提取，新增工作台首页/内容日历/素材库，活动管理增加交付物追踪，竞品追踪增加画像+对比视图 | Artic Workbench |
| 2026-06-01 | v0.5 首页改版为 SaaS 落地页（5屏结构）、全局阴影系统、Sidebar用户区域、毛玻璃工具类 | Artic Workbench |
| 2026-06-01 | v0.6 数据模块重写：文件导入（CSV/XLSX/DOCX via xlsx+mammoth）、数据对比（列映射+差异高亮+导出CSV）、真实AB统计检验（z-test/chi-square/p-value/CI/效应量）、归因数据导入 | Artic Workbench |
